...
(define (feasible space-list token new-loc player-list)
  (let* ([row (list-ref new-loc 0)]
    ...
    (if (or (equal? new-loc (list-ref (list-ref player-list 0) 0)) (equal? new-loc (list-ref (list-ref player-list 0) 1))) #f (< (- new-val val) 2))))
...
(define (score space-list player-list row col)
...
(define (find-max-build space-list token-cell player-list)
  (let* ([score_list (map (lambda (cell) (if (and (and (> (list-ref cell 0) -1) (> (list-ref cell 1) -1)) (and (< (list-ref cell 0) 5) (< (list-ref cell 1) 5)))
  ...
  (let* ([val (map (lambda (elem)
                     (let* ([new-row (+ (list-ref token 0) (list-ref elem 0))] [new-col (+ (list-ref elem 1) (list-ref token 1))]) 
                       (if (and (and (and (> new-row -1) (> new-col -1)) (and (< new-row 5) (< new-col 5)))
                           ...
                           (score space-list player-list new-row new-col)
                           -200)))
                   (build-list 0 0))])
    (define max_score_token (helper val (build-list (list-ref token 0) (list-ref token 1)) -200 (list -1 -1) #t))
    (list (list-ref max_score_token 0) (find-max-build space-list  (list-ref max_score_token 1) player-list))))
...
(define (make-move space-list player-list turn)
  ...
  (let* ([moves (make_best_move (map (lambda (token) (rate-move space-list token player-list)) (list-ref player-list 0)))]
         ...
         [hash_map (hash 'turn turn
                         'players (change-at player-list 0 token_num token_pos)
                         'spaces (change-at space-list build_row build_col (+ (list-ref (list-ref space-list build_row) build_col) 1)))])
    hash_map))
